...
import { MobileNavMenuComponent } from '../mobile-nav-menu/mobile-nav-menu';
import { TranslatePipe } from '../../i18n/translate.pipe';

@Component({
  selector: 'app-dashboard-nav',
  standalone: true,
  imports: [ReleaseNotesPanelComponent, ReleaseNotesModalComponent, SettingsModalComponent, MobileNavMenuComponent, TranslatePipe],
  template: `
    <div class="nav-grid h-full grid grid-cols-5 gap-4">
      <!-- Left section: col-1, aligned with holiday calendar -->
      <div class="nav-tabs col-span-1 bg-white/80 backdrop-blur-xl rounded-2xl shadow-sm shadow-slate-200/50 border border-white flex items-center px-2 gap-1">
        <button
      ...
      <div class="col-span-4 bg-white/80 backdrop-blur-xl rounded-2xl shadow-sm shadow-slate-200/50 border border-white flex items-center px-4 gap-3">
        ...
        <a
          ...
          class="nav-github flex items-center gap-2 text-slate-500 hover:text-slate-800 transition-colors text-sm"
        >
        ...
        >
          ...
          <span class="nav-label">{{ 'nav.whatsNew' | t }}</span>
          @if (svc.hasUnread()) {
    ...
    @if (mobileMenuOpen()) {
      <app-mobile-nav-menu (close)="closeMobileMenu()" />
    }
  `,
  styles: [`
    /* Hamburguesa solo en móvil; resto de la navbar intacto en escritorio. */
    .nav-burger { display: none; }
    .nav-burger svg { width: 1.375rem; height: 1.375rem; }
    @media (max-width: 639.98px) {
      ...
      .nav-burger:hover { color: var(--tt-text); }
    }
  `],
})
